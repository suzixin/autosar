...
- 静态成员
静态成员是属于一个类的，可以在一个类的多个实例之间共享。
用静态函数来访问静态成员，静态函数里只能访问静态变量或外部的全局变量。
非静态成员函数必须使用一个特定的实例来调用。
类里面写的数据成员只是声明，需要在类外面写一个定义，才可以被使用。


- 友员函数
在类中声明一个友元函数，这个函数就可以访问这个类中的私有成员。
...
```cpp
// 这是拷贝构造，对应的函数是Person(const Person &per)
Person p2 = p1;

// 这是运算符重载，对应的函数是Person& operator=(const Person& p)
Person p3;
p3 = p1;
```
